...
int Repository::getByTitle(std::string title)
{
    int searched_index = -1;
    std::vector<Tutorial>::iterator it;
    it = std::find_if(this->adminRepoVector.begin(), this->adminRepoVector.end(), [&title](Tutorial& t) {return t.getTitle() == title;});
    if(it != this->adminRepoVector.end())
        searched_index = it - this->adminRepoVector.begin();
