...
class Game:
    ...
    def move_computer(self):
        # return self._strategy.move_random(self._board)
...
class MoveStrategy:
    ...
    def get_column_with_3_pieces_of_player(self):
        """
         If there if a column on which the player has placed 3 pieces, this function returns it.
         Otherwise it returns None
        :return:
        """
        for column in range(7):
            if not self.column_is_full(column) and (self._board.get_board()[5][column] ==
                    self._board.get_board()[4][column] == self._board.get_board()[3][column] == '   %   ' or
                    self._board.get_board()[4][column] == self._board.get_board()[3][column] ==
                    self._board.get_board()[2][column] == '   %   ' or
                    self._board.get_board()[3][column] == self._board.get_board()[2][column] ==
    ...
    #         column = random.randint(0, 6)  # generates a random column where to move
    #         for row in range(5, -1, -1):
    #             if board.get_position(row, column) == 0:
    #                 return row, column
