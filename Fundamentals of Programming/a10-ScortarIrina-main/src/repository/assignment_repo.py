...
from src.domain.data_structure import Iterable
...
class AssignmentRepository(object):
    def __init__(self):
        self._list_of_assignments = Iterable()

    def generate_assignments(self):
        descriptions = ["test functions", "modular programming", "layered architecture", "procedural programming",
                        "command-based ui", "menu-driven ui", "implement classes", "your own exceptions"]
    ...
    def get_list_of_assignments(self):
        return self._list_of_assignments.get_list()
    ...
    def remove_assignment_by_id(self, assignment_id):
        if type(assignment_id) is str and (not assignment_id.isnumeric() or float(assignment_id) != int(assignment_id)):
...
class AssignmentTextFileRepository(AssignmentRepository):
    def __init__(self):
        super().__init__()
        self._file_name = r"C:\Users\Irina\Desktop\PycharmProjects\a10-ScortarIrina\src\files\text_files\assignments.txt"
        self._clear_file()
        self._load_file()
    ...
    def _save_file(self):
        ...
        for assignment in self._list_of_assignments:
            file.write('assignment id: ' + str(
                assignment.id) + '       description: ' + assignment.description + '       deadline:' + str(
                assignment.deadline) + "\n")

        file.close()

    # function which clears the content of the file every time the program starts
...
class AssignmentBinaryFileRepository(AssignmentRepository):
    def __init__(self):
        super().__init__()
        self._file_name = r"C:\Users\Irina\Desktop\PycharmProjects\a10-ScortarIrina\src\files\text_files\assignments.bin"
        self._clear_file()
        # self._load_file()
