...
class GradeService:
    ...
    def give_assignment_to_student(self, assignment_id, student_id):
        ...
        # self._undo_redo.add(Operation(Call(self._grade_repository.delete_assignment_of_student, assignment_id,
        #                                    student_id),
    ...
    def give_assignment_to_group(self, assignment_id, group_number):
        ...
        for student in self._student_repository.get_list_of_students():
            if int(student.group) == int(group_number) and \
                    self.student_does_not_have_given_assignment(assignment_id, student.id):
    ...
    def get_list_of_students_with_given_assignment_ordered_by_grade(self, assignment_id):
        ...
        if self._grade_repository.get_assignment_by_id(assignment_id) not in \
                self._grade_repository.get_list_graded_assignments():
    ...
    def get_students_with_pass_deadline_assignments(self):
        ...
        for grade in list_of_ungraded_assignments:
            assignment = self._assignment_repository.get_assignment_by_id(grade.assignment_id)
            student = self._student_repository.get_student_by_id(grade.student_id)
            if Iterable.filter_past_deadline(assignment) and student is not None and assignment is not None:
    ...
    def get_list_average_grade_all_students(self):
        ...
        for grade in self._grade_repository.get_list_graded_assignments():
            if grade.student_id not in added_student_ids:
                list_average_grade_all_students.append('student id: ' + str(grade.student_id) + '     average grade: ' +
                                                       str(self.get_average_of_student_grades(grade.student_id)))
